...
def read_any_bytes(name: str, raw: bytes) -> str:
...
def _embed_pdf(raw: bytes, height: int = 520):
...
def preview_file(name: str, raw: Optional[bytes], extracted_text: str):
    ...
    if extracted_text:
        show_full = st.toggle("Show full extracted text", value=False, key=f"toggle_{name}")
        if show_full:
            st.text_area("Extracted text", extracted_text, height=300, key=f"text_{name}")
        else:
            snippet = extracted_text[:2000]
            st.text_area("Extracted text (first 2,000 chars)", snippet, height=220, key=f"text_snip_{name}")
...
# -------- Build preview tabs & combined texts -------- #
st.markdown("---")
...
with prev_spec:
    ...
    if spec_bytes is not None and spec_file is not None:
        try:
            spec_text_from_file = read_any_bytes(spec_file.name, spec_bytes)
        except Exception as e:
            st.warning(f"Failed to read spec file: {e}")
    combined_spec_text = (spec_text_from_file + "\n" + (spec_text_area or "")).strip()
...
with prev_sub:
    ...
    if sub_bytes is not None and sub_file is not None:
        try:
            sub_text_from_file = read_any_bytes(sub_file.name, sub_bytes)
        except Exception as e:
            st.warning(f"Failed to read submittal file: {e}")
    combined_sub_text = (sub_text_from_file + "\n" + (sub_text_area or "")).strip()
    preview_file(sub_file.name if sub_file else "(no file)", sub_bytes, combined_sub_text)

# -------- Analysis button uses the previewed text -------- #
if st.button("Analyze", type="primary"):
    try:
        spec_text = clean_text(combined_spec_text)
        sub_text = clean_text(combined_sub_text)
