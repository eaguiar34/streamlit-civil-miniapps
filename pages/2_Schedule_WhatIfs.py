...
st.success(
f"Fast-tracked duration: {ft.total_duration} days (with {int(overlap_fraction * 100)}% overlap)."
)
...
st.warning(
f"Target not fully met. Best via crashing: {crashed_res.total_duration} days. Est. extra cost ~ ${extra_cost:,.0f}."
)
